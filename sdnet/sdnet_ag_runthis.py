...
class MEncoder(nn.Module):
    def __init__(self, in_channel=3, a_channel=8, z_length=16, img_size=256):
        ...
        self.norm = nn.BatchNorm1d(32)                          # original is BatchNorm
    ...
    def forward(self, img):
        ...
        """
        # out = torch.cat([ana, img], 1)          # concat img [1c] with anantomy facotr [8c]
        out = self.block1(img)
        out = self.block2(out)
        out = self.block3(out)
        out = self.block4(out)
...
class SDNet(nn.Module):
    ...
    def forward(self, x, script_type):
        a_out = self.a_encoder(x)  # encode anatomy factors [bs, 8, 256, 256]
        ...
        if script_type == 'training':
            ...
            _, mu_out_tilede, _ = self.m_encoder(a_out, reco)
        else:
            reco = self.decoder(a_out, mu_out)
            mu_out_tilede = mu_out

        return reco, z_out, mu_out_tilede, a_out, seg_pred, mu_out, logvar_out
